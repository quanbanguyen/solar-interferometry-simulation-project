...
def make_dirty_image(u, v, w, vis, fov_deg=3.0, grid=256, do_fringe_stop=True,
                     dtype=np.complex64, block=8192, time_step=1):
    """Dirty image qua DFT trực tiếp + Hermitian, BLOCKED MATMUL — bộ nhớ thấp.
    Thay einsum nguyên khối (ngốn RAM -> tràn pagefile ổ C) bằng matmul theo khối:
        dirty = Σ_blocks  exp(i·v·m)ᵀ_blk · (V · exp(i·u·l))_blk
    dtype=complex64 giảm nửa RAM; block giới hạn đỉnh RAM; time_step lấy thưa thời gian.
    Trả về (image, l_deg, m_deg)."""
    if time_step > 1:                       # lấy thưa theo thời gian (trục 0) để nhẹ hơn
        u, v, w, vis = u[::time_step], v[::time_step], w[::time_step], vis[::time_step]
    ...
    lvec, mvec = 2 * np.pi * l, 2 * np.pi * m
    K = uf.size
    dirty = np.zeros((grid, grid), dtype=np.complex128)           # tích luỹ chính xác
    for s in range(0, K, block):
        e = min(s + block, K)
        A = np.exp(1j * np.outer(vf[s:e], mvec)).astype(dtype)               # (blk, grid)  [k, m]
        B = (Vf[s:e, None] * np.exp(1j * np.outer(uf[s:e], lvec))).astype(dtype)  # (blk, grid) [k, l]
        dirty += (A.T @ B)                                         # (m, l)
...
# ════════════════════════════════════════════════════════════════════════════
# 10. SELF-TEST / DEMO — chạy `python interfsim.py` để kiểm tra engine
# ════════════════════════════════════════════════════════════════════════════
if __name__ == "__main__":
    print("=" * 64)
    print("interfsim self-test — engine giao thoa kế 2D")
    print("=" * 64)
    obs = Observation(lat_deg=-21.8, dec_deg=14.4,
                      H_deg=np.linspace(-84.2, 84.2, 400))
    print(f"lambda = {obs.lam:.4f} m  | dec = {obs.dec_deg} deg\n")

    comp, ext = preset_compact(), preset_extended()
    print(comp.summary())
    print(ext.summary())

    sun = {'type': 'disk', 'flux': 1.0, 'diameter_rad': np.radians(0.53)}
    for cfg in (comp, ext):
        u, v, w, V = forward_visibilities(cfg, obs, sun)
        psf, ld, md = synthesized_beam(u, v, w, fov_deg=3, grid=192, do_fringe_stop=False)
        row = psf[psf.shape[0] // 2]
        idx = np.where(row >= 0.5)[0]
        fwhm = (ld[idx.max()] - ld[idx.min()]) * 60 if idx.size else float('nan')
        print(f"  {cfg.name:26s} | bl_max={cfg.baseline_lengths.max():6.1f} m"
              f" | PSF main-lobe (l) ~ {fwhm:5.1f} arcmin")
    ...
    try:
        ...
        for ax, cfg in zip(axs, (comp, ext)):
            u, v, w, V = forward_visibilities(cfg, obs, sun)
            img, l, m = make_dirty_image(u, v, w, V, fov_deg=2.2, grid=220, do_fringe_stop=False)
            ax.imshow(img, extent=[l[0]*60, l[-1]*60, m[0]*60, m[-1]*60],
                      origin="lower", cmap="hot", vmin=0, vmax=1)
            th = np.linspace(0, 2*np.pi, 200)
            ax.plot(16*np.cos(th), 16*np.sin(th), "c--", lw=1)
            ax.set_title(cfg.name); ax.set_xlabel("E-W (arcmin)"); ax.set_ylabel("N-S (arcmin)")
        plt.tight_layout(); plt.savefig("interfsim_selftest.png", dpi=90)
        print("\nĐã lưu interfsim_selftest.png — so sánh dirty image compact vs extended.")
